...
data$dummy_coast <- as.factor(data$dummy_coast)
...
categorical_variables <- c("municipality", "county", "coast")
...
# 
# # Save to CSV
# # write.csv(final_table, "tables/summary_statistics_mountains.csv", row.names = FALSE)
# 
# # Print the summary table
# print(summary_table)
# glimpse(summary_table)
...
pdf(file = "tables/correlations_all_variables_mountains.pdf", width = 16.5, height = 11.7)
corrplot(correlations, method = 'number', order = "hclust", type = "lower", 
title = "Kendall Correlation Matrix, all variables, mountain municipalities", 
mar = c(0,0,1,0), number.cex = 0.5, number.digits = 2, tl.cex = 0.7)

dev.off()

write.csv(correlations, file = "tables/correlation_all_variables_mountains.csv", row.names = FALSE)
...
write.csv(summary_cor_with_response, file = "tables/correlations_with_response_variables_mountain.csv", row.names = FALSE)
...
# Selecting the first four axes
hytter_pc1_pc2_pc3_pc4 <- hytter_pca |> 
  filter(component == "PC1" | component == "PC2" | component == "PC3" | component == "PC4")

# Open PDF device
pdf("plots/PCA_loadings_Holiday_Homes_in_Mountain_Regions_in_Norway.pdf", width = 8.27, height = 11.69) # A4 size in inches
...
hytter_pc1_pc2_pc3_pc4 %>%
  ...
  labs(
    ...
    y = NULL,
    fill = "Positive?",
    title = "Loadings by principal component 1-4\nHoliday homes in Mountain regions in Norway"
  ) 
...
# Save to CSV
write.csv(pca_wider, "tables/pca__loadings_holiday_homes_mountains.csv", row.names = FALSE)
...
fig_s_pca_mountain_with_names <- juice(hytter_recipe) %>%
  ggplot(aes(PC1, PC2, label = municipality)) +
  geom_point(aes(color = county), alpha = 0.7, size = 3) +
  geom_text(color = "grey1", check_overlap = FALSE,  vjust = "bottom", nudge_y = 0.1, family = "IBMPlexSans") +
  labs(color = NULL)+
	  theme_minimal()

fig_s_pca_mountain_with_names


# Open PDF device


# cbp10 <- c("#1f78b4", "black", "cadetblue4", "#33a02c", "#fb9a99", "#e31a1c", 
# 	"#fdbf6f", "#ff7f00", "darkblue", "#6a3d9a")
# 
# # Create the plot
# fig_s_pca_mountains_with_names <- juice(hytter_recipe) %>%
#   ggplot(aes(PC1, PC2, label = municipality)) +
#   geom_point(aes(color = county), alpha = 1, size = 3) +
#   #scale_colour_manual(values=cbp10)+
#   geom_text(aes(color = "black"), check_overlap = FALSE, vjust = "bottom", nudge_y = 0.05, family = "IBMPlexSans") +
#   labs(
#     title = "PCA Plot by Municipality and County",
#     color = NULL
#   ) +
#   xlim(-6, 8) +
#   ylim(-5, 6) +
#   theme_minimal()
# 
# fig_s_pca_mountains_with_names 
# 
# # Close PDF device
# dev.off()
...
arrows_updated <- arr %>%
  mutate(terms_updated = case_when(
    ...
    terms == "pop_4hr_drive" ~  "Pop. 4hr drive",
    terms == "holiday_homes" ~  "Holiday homes",
    terms == "holiday_homes_1970" ~  "Holiday homes 1970",
    terms == "activity_pr_person" ~  "Activity pr person",
    #terms == "prop_montainous" ~  "Mountain area",
    terms == "median_income" ~  "Median income",
    terms == "dist_to_coast" ~  "Inland",
...
fig_pca_mountain <- juice(hytter_recipe) %>%
  ggplot(aes(PC1, PC2)) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "darkgrey") +
  geom_hline(yintercept = 0, linetype = "dashed", color = "darkgrey") +
  geom_point(aes(fill = county), shape = 21, color = "grey30", size = 3, alpha = 1) +
  ...
  theme_bw() +
  xlab("Principal component 1 (20.6 % variation explained)") +
  ylab("Principal component 2 (15.6 % variation explained)") +
...
fig_pca_mountain <- juice(hytter_recipe) %>%
  ...
  geom_segment(data = arrows_updated,
               ...
               arrow = arrow(length = unit(1/2, "picas")),
               color = "grey39") +
  geom_text_repel(data = arrows_updated,
                  aes(x = PC1 * 17, y = PC2 * 17, label = terms_updated),
                  size = 4, 
                  box.padding = 0.5,
                  point.padding = 0.5) +
  labs(fill = "New Legend Title") +  # Change the legend title
...
print(fig_pca_mountain)

ggsave("plots/fig_pca_mountain.png", 
	fig_pca_mountain, scale = 2, bg = "white", width = 15, height = 10, units = "cm", dpi = 600)
...
ggsave("plots/fig_s_pca_mounain_with_names.png", 
	fig_s_pca_mountain_with_names, scale = 2, bg = "white", width = 15, height = 10, units = "cm", dpi = 600)
